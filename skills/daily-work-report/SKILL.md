...
description: '汇总生成日/周工作报告，跨所有项目、workspace、对话与 git 历史。使用场景：用户要求"总结昨天/本周/某几天的日报"、"汇总这段时间做了什么"、"输出工作汇报"，或需要跨多个仓库归纳提交、会话与改动。数据源为 VS Code Copilot 本地会话库（session-store.db）与云端会话存储（覆盖跨设备会话），辅以 git 历史与记忆；非 VS Code 平台的降级方案见参考文档。'
