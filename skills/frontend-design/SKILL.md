...
description: '前端设计品味与交互规范（桌面+移动端通用）。使用场景：新建/美化/重构页面或组件、"设计得有辨识度"、"好看点但别像 AI 模板"、UI 评审、改造已有界面的 AI 味、要求"不要千篇一律"。核心流程：先读需求定调（布局气质/动效强度/信息密度三轴），再执行反 AI 味设计原则，交付前过自检清单。方法论主体为 taste-skill，落地范例为自有记账应用的真实项目。'
...
- 暗色模式提亮提饱和（自有项目验证），避免近黑吞掉主题色；阴影染底色，不纯黑投影
